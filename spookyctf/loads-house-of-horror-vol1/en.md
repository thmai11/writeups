...
We have `Dockerfile` `watchdog.py` that I have not used. The only file I needed was `welcome.py`
...
if __name__ == "__main__":
    main()
```
So I get that when I initiate the connection, the server will change the flag location randomly

# Injection point

Place where the flag could be
...
```
Also I notice the use of `subprocess.chek_output` with user input, that might be risky :)
...
The server is trying to sanitize the input in the `converse` method
```python
if any(char in response for char in [";", "&", "|", "`", "$", ">", "<"]):
    with open("/tmp/injection_detected", "w") as f:
        f.write("1")
...
So we found the injectoin point that I try with the usual `id` comamnd

If we just read all the possible flag location with a command like this
```bash
