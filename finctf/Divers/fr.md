...
## Flag 1: L'Ascension de la Tour 🏰

On a un fichier zip contenant je n'ai aucune espece d'idee de combien de subfolder... mais... ce n'est pas mon premier jour sur Linux!
...
Nous avons le fichier suivant:
![](img/Impressions_Cachees.png)

Je ne suis pas Sherlock Holme mais il y a un flag dans ce fichier!
Je lance donc `docker.io/dominicbreuker/stego-toolkit` https://github.com/DominicBreuker/stego-toolkit

Resultat: `Rien du tout`

Donc je me lance sur une recherche RGB/LSB avec https://georgeom.net/StegOnline/image
LSB Half et j'obtiens...

![](img/solution.png)

## Flag 3: L'unite centrale
On a la string suivante:
```
...
Dans Cyberchef utilisee Magic
![alt text](img/central.png)
...
D5, f
```
Il faut comprendre que la ligne sur laquelle la cle de sol se retrouve est G4
Donc si vous n'avez aucune connaissance musicale simplement Partir de sol (G4) et de compter do re mi fa sol la si do. Le chiffre correspond a l'octave 
Voici la correspondance entre les notes et la lettre qui les representes
```
do C
re D
mi E
...
do C
```
Convertir le HEX en ASCII.

# Flag 5: Enregistrements Enigmatiques 🔊
Nous avons des fichier audio nommer selon des characteres ASCII.
Nous devons faire en sorte que le son aigu derriere soit contigue,

Simplement les mettres dans audacity en vue spectrogram
![alt text](img/noise.png)
L'ordre des fichiers vous donne le FLAG!

# Flag 6: Coffre-Fort 🔒
Une archive zip proteger par mot de passe. 4 chiffre.
Pour celui-ci j'ai simplement demander a chatGTP un script pour le brute force
```python
import zipfile
...
```bash
python solve.py 
Mot de passe trouvé: 4856
```

# Flag 7: Pages Fugaces ⌛️
Nous avons le gif suivant:  
<details>

![](img/Pages_Fugaces.gif)

</details>

Un site comme https://ezgif.com/split/ peut vous le montrer frame par frame
![alt text](img/frame.png)



# Flag 8: Artefact
Cet "artefact" contient un gros paquet de fichier bidon contenant du lorem ipsum.
...
Rg eâqyz kqfav gck wsj btqusj yipl zv xqge.
...
Bon bon bon, ces challenges de crypto/encodage ouf, que puis-je deduire
```
vkoxu://vmszzeasw.dw/
```
Uniquement les A-Za-z0-9 semble etre utilisee dans le chiffrage
Et ceci ressemble clairement a une url non?

Je vais donc sur https://www.boxentriq.com/code-breaking/cipher-identifier
Je teste differente options et

![alt text](img/cipher.png)

Oh! victor semble proche. 
Je joue avec les charactere et avec `Victor` sa fonctionne
...
Est toujours non-resolue et je commence toujours par tester les plus commun un simple ROT13
```
...
Ainsi que la derniere phrase du poeme
key: Poème de qui ?? IV: 'Victor Hugo    :)'
```

AWW oui l'autre fichier.. et bien on a la recette...
CyberChef!